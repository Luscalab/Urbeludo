...
function BalanceGame({ onWin, auraColor }: { onWin: (reward: number, type: string) => void, auraColor: string }) {
  ...
  // Níveis ajustados: margem de erro (threshold) muito mais generosa nos primeiros níveis
  const BALANCE_PHASES = [
    { name: 'Ninho de Luz', threshold: 120, targetSize: 100, ballSize: 80, reward: 20, benefit: "Sinta seu corpo no espaço com leveza." },
    { name: 'Pulo do Gato', threshold: 80, targetSize: 90, ballSize: 75, reward: 30, benefit: "Controle postural e estabilidade básica." },
    { name: 'Mestre da Árvore', threshold: 40, targetSize: 80, ballSize: 70, reward: 50, benefit: "Precisão absoluta e foco meditativo." }
  ];
  ...
  useEffect(() => {
    ...
    // Orquestra Digital: Sons mais suaves e harmônicos
    ...
    const timer = setInterval(() => {
      if (isInside) setProgress(prev => Math.min(100, prev + 2)); 
      else setProgress(prev => Math.max(0, prev - 0.5)); 
    }, 100);
...
function VoiceGame({ onWin, auraColor, ludoCoins }: { onWin: (reward: number, name: string) => void, auraColor: string, ludoCoins: number }) {
  ...
  useEffect(() => {
    ...
    // Lógica Matemática de Normalização Biomecânica
    // h = total_height * ( (vol - min) / (max - min) )
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center relative overflow-hidden bg-slate-950">
      {/* 1. FUNDO - Caminho Relativo para APK */}
...
function RhythmGame({ onWin, auraColor }: any) {
  ...
  }, [hits, phase, onWin, currentLevel.goal, currentLevel.reward]);
